# ============================================================
# VIDEO MONETIZATION TELEGRAM BOT - RENDER OPTIMIZED (FIXED)
# ============================================================
...
# ============================================================
# INITIALIZE BOT - FIXED FOR RENDER
# ============================================================

# FIX: in_memory=False use karo (default) taki session file properly save ho
# Render par session file kaam karegi kyunki /opt/render/project/src/ persistent hai
...
async def auto_delete_messages(chat_id: int, message_ids: list, delay_minutes: int):
    ...
    for msg_id in message_ids:
        try:
            ...
            logger.error(f"Delete failed msg {msg_id}: {e}")
...
async def start_command(client: Client, message: Message):
    ...
    sent_msg = await message.reply_text(welcome_msg)
...
async def verify_join_callback(client: Client, callback_query: CallbackQuery):
    ...
    logger.info(f"Verify join callback from user {user_id}")
...
async def handle_health(request):
    """Render health check endpoint - bot ke kaam par koi asar nahi"""
    return web.Response(text="OK", status=200)

async def run_health_server():
    """Render ke health check ke liye minimal web server"""
...
async def main():
    ...
    # FIX: Thoda sa wait karo sab kuch initialize hone ke liye
    await asyncio.sleep(1)
    ...
    # FIX: Bot start hone ke baad thoda wait karo
    ...
    # FIX: Bot ko forcefully idle mode mein daalo taki updates properly receive ho
